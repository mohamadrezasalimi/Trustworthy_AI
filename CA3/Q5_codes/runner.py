...
def run_benchmark(models, datasets, seed, N_explain):
    ...
    # ------------------------------------------------------------------------------------------------------------------
    learned_scms = {'adult': scm.Learned_Adult_SCM, 'compas': scm.Learned_COMPAS_SCM, 'health': None}  # اضافه کردن health
    for dataset in datasets:
        for model_type in ['lin', 'mlp']:
            # We only need to learn the SCM for some of the data sets
            if dataset in learned_scms.keys() and learned_scms[dataset] is not None:
...
if __name__ == "__main__":
    # Define the models and datasets you want to run
    models = ['lin', 'mlp']
    datasets = ['adult', 'compas', 'health']
    seed = 0
    N_explain = 100

    run_benchmark(models, datasets, seed, N_explain)
