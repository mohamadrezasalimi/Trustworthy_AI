...
def process_health_data():
    ...
    # ویژگی‌های قابل اقدام: only blood glucose and insulin
    actionable = [1, 2]  # indices of insulin and blood_glucose

    # محاسبه حداکثر و حداقل مقادیر
    feature_limits = np.array([[X_health[:, i].min(), X_health[:, i].max()] for i in range(X_health.shape[1])])
